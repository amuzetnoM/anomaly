...
import { CipherDisplay } from "@/components/cipher/cipher-display"
import { CipherInput } from "@/components/cipher/cipher-input"
  ...
  useEffect(() => {
    ...
    if (bookData) {
      try {
        ...
        const foundCipher = parsedData.ciphers?.find((c: any) => c.id === params.id)

        if (foundCipher) {
          setCipher(foundCipher)

          // Check if already solved
          const isSolved = Object.keys(realityState.memories).some((key) => key === `cipher-${foundCipher.id}-solved`)
    ...
    return (
      <div className="container mx-auto py-8">
        ...
        <div className="flex items-center justify-center h-[60vh]">
          <p className="text-muted-foreground">Loading cipher...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <CipherDisplay cipher={cipher} showSolution={solved} />

        <CipherInput cipherId={cipher.id} solution={cipher.solution || "solution"} onSolved={handleSolved} />
      </div>
    </div>
  )
}
